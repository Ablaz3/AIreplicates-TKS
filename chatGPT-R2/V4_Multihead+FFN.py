...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        return torch.cat([h(x) for h in self.heads], dim=-1) #concatenate these heads
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        #define a self attention head for the model
        self.sa_heads = MultiHeadAttention(4, n_embd//4) #h heads of size h/4. // b.c integer division
        #create a feedforward property
        self.ffwd = FeedFoward(n_embd) #n_embd is just the dimensionality of the input tensor
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.sa_heads(x) # compute self attention
        x = self.ffwd(x) # (B, T, C)
